...
#
# The output is chained: it includes the wrapper-adoption hop (this repo's
# moved.tf re-anchored under module.tenant.), and tofu follows the chain, so
# the same migration PR plans clean whether or not the tenant ever applied the
# wrapper refactor. Verified from both starting states: moves only, 0/0/0.
#
# Assumptions:
...
if [ $# -eq 0 ]; then
  ...
  if grep -qE '^module "tenant"' tenant.tf; then
    echo 'tenant.tf still declares module "tenant" — delete the wrapper call first' >&2
    exit 1
  fi
...
# hop 2: wrapper-form -> per-cluster blocks
