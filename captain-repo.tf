...
module "provider_versions" {
  source = "git::https://github.com/GlueOps/terraform-module-provider-versions.git?ref=v0.3.3"
}
...
module "captain_repository_files" {
  ...
  files_to_create = {
    ...
    "terraform/gluekube/.tfvars"                         = try(module.generate_gluekube_creds[each.value.environment_name].gluekube_tfvars, "")
    "terraform/gluekube/variables.tf"                    = try(module.generate_gluekube_creds[each.value.environment_name].gluekube_variables_tf, "")
...
module "configure_vault_cluster" {
    source = "git::https://github.com/GlueOps/terraform-module-kubernetes-hashicorp-vault-configuration.git?ref=v0.12.0"
