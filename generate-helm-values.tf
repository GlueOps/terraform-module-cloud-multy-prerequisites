...
module "glueops_platform_helm_values" {
  ...
  source                                     = "git::https://github.com/GlueOps/platform-helm-chart-platform.git?ref=feat/update-images-to-use-registry-caches"
...
module "argocd_helm_values" {
  for_each             = local.environment_map
  source               = "git::https://github.com/GlueOps/docs-argocd.git?ref=feat-Update-image-repositories-in-argocd.yaml.tpl-to-registry-caches"
  tenant_key           = var.tenant_key
  cluster_environment  = each.value.environment_name
