...
module "glueops_platform_helm_values" {
  ...
  source                                     = "git::https://github.com/GlueOps/platform-helm-chart-platform.git?ref=v0.79.2"
...
module "argocd_helm_values" {
  for_each             = local.environment_map
  source               = "git::https://github.com/GlueOps/docs-argocd.git?ref=feature/global-otel-backend"
  tenant_key           = var.tenant.tenant_key
  cluster_environment  = each.value.environment_name
