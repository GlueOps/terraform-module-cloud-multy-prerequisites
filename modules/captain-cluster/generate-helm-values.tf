...
module "argocd_helm_values" {
  for_each             = local.environment_map
  source               = "git::https://github.com/GlueOps/docs-argocd.git?ref=v0.22.0" # OTEL UI extension frontend: server.extensions installer, extension.config, and the extension RBAC scoped to otel-extension
  tenant_key           = var.tenant.tenant_key
  cluster_environment  = each.value.environment_name
